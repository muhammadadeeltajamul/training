...
class WeatherData:
    ...
    def add_month_data(self, month: int, year: int, 
        ...
        month = str(month)
        year = str(year)
        if not year in self.__complete_data:
            self.__complete_data[year] = {}
        if not month in self.__complete_data[year]:
    ...
    def get_column_data(self, column_name, month: int, year: int):
        ...
        try:
            ...
            pass
        #print(month, year, "DATA NOT FOUND")
        return []
...
def minus_c(weather_data: WeatherData, month: int, year: int):
    ...
    if max_len != min_len:
        if min_len < max_len:
        	len_ = min_len
        print("\033[1;31;40m Note: Unequal data of maximum and minimum temperature")
...
def list_all_files(path):
    ...
    for file in os.listdir(path):
        if file.startswith(".") or len(file) < 24:
    	    print(file, "File not load because month and year could not be extracted")
    	    flag = True
    	    continue
        name_split = os.path.splitext(file)[0].split('_')
        #print("Loading", name_split[3], name_split[2])
        try:
            year = int(name_split[2])
            month = int(get_month_number_from_string(name_split[3]))
        except:
            print("File not load because month and year could not be extracted")
            continue
        if year == "" or month == "":
            print(file, "File not load because month and year could not be extracted")
            continue
...
if __name__ == "__main__":
    ...
    parser.add_argument('-e', action='append', type=str, 
                           help="Displays highest temperature, lowest temperature and humidity of a year")
    parser.add_argument('-a', action='append', type=str, 
                           help="Displays average highest temperature, average lowest temperature and average humidity of a month")
    parser.add_argument('-c', action='append', type=str, 
                           help="Draws highest and lowest temperature of a month")
